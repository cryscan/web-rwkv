use crate::serde::deprecated::allow_deprecated;
use crate::serde::fragment::{Expr, Fragment, Stmts};
use crate::serde::internals::ast::{Container, Data, Field, Style, Variant};
use crate::serde::internals::name::Name;
use crate::serde::internals::{attr, replace_receiver, ungroup, Ctxt, Derive};
use crate::serde::{bound, dummy, pretend, private, this};
    ...
    let impl_block = if let Some(remote) = cont.attrs.remote() {
        ...
        quote! {
            ...
            impl #de_impl_generics #ident #ty_generics #where_clause {
                #vis fn deserialize<__D>(self, __deserializer: __D) -> _serde::#private::Result<#remote #ty_generics, __D::Error>
                where
        ...
        quote! {
            ...
            impl #de_impl_generics #serde::de::DeserializeSeed<#delife> for #seed<#delife, #context, #ident #ty_generics> #where_clause {
                ...
                fn deserialize<__D>(self, __deserializer: __D) -> #serde::#private::Result<Self::Value, __D::Error>
                where
                ...
                Some(path) => {
                    ...
                    quote!({
                        ...
                        _serde::#private::Option::map(
                            _serde::de::SeqAccess::next_element_seed::<#seed<#context, #wrapper_ty>>(&mut __seq, #seed::new(self.context))?,
                            |__wrap| __wrap.value)
                    })
                }
            };
