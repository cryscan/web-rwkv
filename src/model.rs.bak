...
use crate::{
    tensor::{CpuFp16, GpuFp16, GpuFp32, GpuInt8, Tensor},
    Context,
};
...
pub enum TensorQuant<'a> {
    Fp16(Tensor<'a, GpuFp16>),
    Int8 {
        w: Tensor<'a, GpuInt8>,
        mx: Tensor<'a, GpuFp32>,
        my: Tensor<'a, GpuFp32>,
...
struct Att<'a> {
    time_decay: Tensor<'a, GpuFp32>,
    time_first: Tensor<'a, GpuFp32>,

    time_mix_k: Tensor<'a, GpuFp16>,
    time_mix_v: Tensor<'a, GpuFp16>,
    time_mix_r: Tensor<'a, GpuFp16>,
...
struct Embed<'a> {
    ...
    w: Tensor<'a, CpuFp16<'a>>,
}

struct Head<'a> {
    layer_norm: LayerNorm<'a>,
    w: Vec<Tensor<'a, GpuFp16>>,
}
...
impl<'a> ModelBuilder<'a> {
    ...
    pub fn build(self) -> Result<Model> {
        ...
        let tensor = ModelTensor {
            num_emb: create_uniform_u32(&[num_emb as u32]),
            num_vocab: create_uniform_u32(&[num_vocab as u32]),
            embed,
            head,
            layers,
        };
        ...
        let embed = {
            let layer_norm = device.create_bind_group(&BindGroupDescriptor {
                ...
                entries: &[
                    BindGroupEntry {
                        binding: 0,
                        resource: self.tensor.num_emb.as_entire_binding(),
                    },
                    BindGroupEntry {
                        binding: 1,
    ...
    fn create_softmax_bind_group(&self, buffer: &ModelBuffer) -> BindGroup {
        ...
        device.create_bind_group(&BindGroupDescriptor {
            ...
            entries: &[
                BindGroupEntry {
                    binding: 0,
                    resource: self.tensor.num_vocab.as_entire_binding(),
                },
                BindGroupEntry {
                    binding: 1,
                    resource: buffer.num_tokens.as_entire_binding(),
